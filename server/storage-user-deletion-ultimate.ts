...
export async function deleteUserUltimate(userId: string): Promise<void> {
  ...
  try {
    ...
    const vendorCheck = await safeExecute(
      "vendor check",
      sql`
    ...
    if (vendorCheck) {
      ...
      if (vendorResult.rows.length > 0) {
        ...
        await safeExecute(
          "review_votes for vendor products",
          sql`
        ...
        await safeExecute(
          "review_responses for vendor products",
          sql`
        ...
        await safeExecute(
          "cart items for vendor products",
          sql`
          DELETE FROM cart 
          WHERE product_id IN (
            SELECT id FROM products WHERE vendor_id = ${vendorId}
        ...
        await safeExecute(
          "order_items for vendor products",
          sql`
    ...
    await safeExecute(
      "review_responses for user reviews",
      sql`
      DELETE FROM review_responses 
