...
export class OrderNotificationService {
  ...
  async sendOrderConfirmation(data: OrderNotificationData): Promise<boolean> {
    if (!data.order.customer?.email) {
      console.warn(`Cannot send order confirmation: customer email missing for order ${data.order.id}`);
      return false;
    }
    ...
    try {
      ...
      if (success) {
        console.log(`✅ Order confirmation sent to ${data.order.customer.email} for order ${data.order.orderNumber}`);
      }
   ...
   */
  async sendStatusUpdate(data: OrderNotificationData, newStatus: string, message?: string): Promise<boolean> {
    if (!data.order.customer?.email) {
    ...
    try {
      ...
      if (success) {
        console.log(`✅ Status update sent to ${data.order.customer.email} for order ${data.order.orderNumber}: ${statusLabel}`);
      }
  ...
  async sendDriverAssignmentNotification(data: OrderNotificationData): Promise<boolean> {
    if (!data.order.customer?.email || !data.order.driver) {
      console.warn(`Cannot send driver assignment: missing email or driver info for order ${data.order.id}`);
      return false;
    }
    ...
    try {
      ...
      if (success) {
        console.log(`✅ Driver assignment sent to ${data.order.customer.email} for order ${data.order.orderNumber}`);
      }
  ...
  async sendVendorOrderNotification(data: OrderNotificationData): Promise<boolean> {
    if (!data.order.vendor?.email) {
      console.warn(`Cannot send vendor notification: vendor email missing for order ${data.order.id}`);
      return false;
    }
    ...
    try {
      ...
      if (success) {
        console.log(`✅ Vendor notification sent to ${data.order.vendor.email} for order ${data.order.orderNumber}`);
      }
   ...
   */
  async sendLowStockAlert(vendorEmail: string, vendorName: string, products: Array<{ name: string; quantity: number; threshold: number }>): Promise<boolean> {
  ...
  private generateOrderConfirmationTemplate(data: OrderNotificationData): string {
    const { order } = data;
    const customerName = order.customer ? `${order.customer.firstName} ${order.customer.lastName}` : 'Client';
  ...
  private generateStatusUpdateTemplate(data: OrderNotificationData, statusLabel: string, message?: string): string {
    const { order } = data;
    const customerName = order.customer ? `${order.customer.firstName} ${order.customer.lastName}` : 'Client';
  ...
  private generateStatusUpdateText(data: OrderNotificationData, statusLabel: string, message?: string): string {
    const { order } = data;
    const customerName = order.customer ? `${order.customer.firstName} ${order.customer.lastName}` : 'Client';
  ...
  private generateVendorOrderTemplate(data: OrderNotificationData): string {
    const { order } = data;
    const vendorName = order.vendor?.businessName || `${order.vendor?.firstName} ${order.vendor?.lastName}` || 'Vendeur';
  ...
  private generateLowStockTemplate(vendorName: string, products: Array<{ name: string; quantity: number; threshold: number }>): string {
    return `
      <!DOCTYPE html>
      <html>
      <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
  ...
  private generateLowStockText(vendorName: string, products: Array<{ name: string; quantity: number; threshold: number }>): string {
