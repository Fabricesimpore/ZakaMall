...
export async function deleteUserSafe(userId: string): Promise<void> {
  ...
  try {
    ...
    await safeDelete("user reviews", () => db.delete(reviews).where(eq(reviews.userId, userId)));
...
export async function deleteProductSafe(productId: string): Promise<void> {
  ...
  try {
    ...
    await safeDelete("reviews", () => db.delete(reviews).where(eq(reviews.productId, productId)));

    // Phase 2: Delete cart items for this product
    await safeDelete("cart items", () => db.delete(cart).where(eq(cart.productId, productId)));
