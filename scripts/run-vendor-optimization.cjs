...
async function runMigration() {
  const connectionString = process.env.DATABASE_URL || 'postgresql://postgres:postgres@localhost:5432/zakamall';
  ...
  try {
    ...
    const optimizationPath = path.join(__dirname, '..', 'migrations', '0002_vendor_optimization.sql');
