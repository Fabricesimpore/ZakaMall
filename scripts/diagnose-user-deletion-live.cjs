...
async function diagnoseUserDeletion(userIdOrEmail) {
  ...
  try {
    ...
    // Find the user
    let user;
    const { rows: userById } = await pool.query('SELECT * FROM users WHERE id = $1', [userIdOrEmail]);
    if (userById.length > 0) {
      user = userById[0];
    } else {
      const { rows: userByEmail } = await pool.query('SELECT * FROM users WHERE email = $1', [userIdOrEmail]);
