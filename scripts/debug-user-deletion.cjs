...
async function debugUserDeletion() {
  ...
  try {
    ...
    for (const constraint of constraints) {
      ...
      try {
        ...
        if (totalRecords > 0) {
          ...
          console.log(
            `  ❌ ${tableName}.${columnName}: ${totalRecords} records referencing ${uniqueUsers} users`
          );
        } else {
    ...
    if (testUserId) {
      ...
      const { rows: userCheck } = await pool.query("SELECT email FROM users WHERE id = $1", [
        testUserId,
      ]);
    ...
    for (const issue of potentialIssues) {
      try {
        ...
        if (!hasExpected && hasActual) {
          console.log(
            `  ⚠️ ${issue.table}: Missing ${issue.expectedColumn}, has ${issue.actualColumn}`
          );
        } else if (hasExpected) {
