...
async function verifyProductsSetup() {
  ...
  try {
    ...
    } = await pool.query("SELECT COUNT(*) as count FROM products WHERE is_active = true");
    console.log(`📦 Total active products: ${totalProducts}`);

    // Check products with vendor information
    const {
      rows: [{ count: productsWithVendor }],
    } = await pool.query(
    ...
    sampleProducts.forEach((product) => {
      ...
      console.log(
        `    Vendor: ${product.vendor_display_name || "Missing"} (ID: ${product.vendor_id})`
      );
    ...
    } = await pool.query("SELECT COUNT(*) as count FROM vendors WHERE status = 'approved'");
    console.log(`🏢 Approved vendors: ${totalVendors}`);

    // Check for orphaned products
    const {
      rows: [{ count: orphanedProducts }],
    } = await pool.query(`
