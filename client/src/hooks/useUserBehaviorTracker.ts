...
export function useUserBehaviorTracker() {
  ...
  const trackViewEnd = useCallback((productId: string, metadata?: Record<string, any>) => {
  ...
  // Track add to cart
  const trackAddToCart = useCallback((productId: string, metadata?: Record<string, any>) => {
    trackBehavior({
      productId,
  ...
  // Track purchase
  const trackPurchase = useCallback((productId: string, metadata?: Record<string, any>) => {
    trackBehavior({
      productId,
      actionType: "purchase",
      metadata,
  ...
  // Track like/favorite
  const trackLike = useCallback((productId: string, metadata?: Record<string, any>) => {
    trackBehavior({
      productId,
      actionType: "like",
      metadata,
  ...
  // Track share
  const trackShare = useCallback((productId: string, metadata?: Record<string, any>) => {
    trackBehavior({
      productId,
      actionType: "share",
      metadata,
  ...
  const trackSearchClick = useCallback((productId: string, searchQuery?: string, position?: number) => {
    trackBehavior({
      productId,
